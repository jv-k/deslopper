...
def test_evaluate_reads_only_the_gateway_key_and_only_at_call_time(monkeypatch):
    """The module is already imported; the key appears now, and nothing else is read."""
    monkeypatch.setattr(jev, "post", _canned(PROBE_RESPONSE))
    read = []
    real_get = os.environ.get

    def spying_get(name, default=None):
        read.append(name)
    ...
    assert set(read) == {"AI_GATEWAY_API_KEY"}
...
def test_post_sends_a_bearer_request_to_the_endpoint_with_the_timeout(monkeypatch):
    ...
    reply = jev.post({"model": jev.MODEL, "state": "s", "questions": QUESTIONS})

    request = seen["request"]
    assert request.full_url == jev.ENDPOINT
    assert request.get_method() == "POST"
    assert json.loads(request.data) == {"model": jev.MODEL, "state": "s", "questions": QUESTIONS}
    # A boolean, so a failure prints `assert False` and never the header.
    header_is_bearer_key = request.get_header("Authorization") == "Bearer " + key
...
def test_free_tier_403_gets_the_top_up_hint(monkeypatch):
    ...
    with pytest.raises(jev.JevError) as excinfo:
        jev.post({"model": jev.MODEL, "state": "s", "questions": QUESTIONS})
...
def test_malformed_json_raises_jev_error(monkeypatch):
    monkeypatch.setenv("AI_GATEWAY_API_KEY", "test-key-not-real")
    _fake_urlopen(monkeypatch, _Reply(b"<html>gateway timeout</html>"))
...
def test_the_key_never_appears_in_an_error_message(monkeypatch):
    ...
    leaked = key in str(excinfo.value) or key in repr(excinfo.value)
    assert not leaked
