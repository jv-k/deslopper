...
# VerBump commits its own changes. Anything left behind means the bump did not finish.
if [ -n "$(git status --porcelain)" ]; then
  echo "working tree is not clean after the bump; VerBump left changes uncommitted" >&2
  exit 1
fi
...
# The version lives in three files and they have to agree. A VerBump that only knows
# package.json leaves pyproject.toml and __init__.py behind, which is how v0.1.2 broke.
"$py" -m pytest -q tests/test_version.py

# Without -c, VerBump rewrites CHANGELOG.md with raw commit subjects. Those land above the
