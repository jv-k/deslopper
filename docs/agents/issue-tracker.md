...
Issues and PRDs for this repo live as GitHub issues in `jv-k/deslopper`. Use the `gh` CLI
for all operations.
...
- **Child ticket:** an issue linked to the map as a GitHub sub-issue (`gh api` on the
  ...
  ticket is assigned to the dev running it.
...
- **Frontier query:** list the map's open children (`gh issue list --state open`, scoped to
  the map's sub-issues or task list), then drop any with an open blocker
...
- **Resolve:** `gh issue comment <n> --body "<answer>"`, then `gh issue close <n>`, then
  append a context pointer (gist and link) to the map's Decisions-so-far.
